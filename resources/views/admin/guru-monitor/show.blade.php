...
<div class="p-6">
    ...
    <div class="flex items-center gap-4 mb-6">
        <a href="{{ route('admin.guru-monitor.index') }}" class="text-gray-400 hover:text-gray-600">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-gray-900">{{ $guru->name }}</h1>
            <p class="text-gray-500 text-sm">{{ $guru->email }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-2xl font-bold text-gray-900">{{ number_format($stats['total_entries']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Excellent</p>
            <p class="text-2xl font-bold text-green-600">{{ number_format($stats['excellent_entries']) }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <p class="text-xl font-bold text-blue-600">Rp {{ number_format($stats['available_balance'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Pending WD</p>
            <p class="text-xl font-bold text-orange-500">Rp {{ number_format($stats['pending_withdrawal'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 p-4">
            <p class="text-xs text-gray-500">Total Dibayar</p>
            <p class="text-xl font-bold text-gray-700">Rp {{ number_format($stats['total_paid_out'], 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            @foreach(['excellent' => ['label' => 'Excellent', 'color' => 'green'], 'good' => ['label' => 'Good', 'color' => 'blue'], 'fair' => ['label' => 'Fair', 'color' => 'yellow'], 'poor' => ['label' => 'Poor', 'color' => 'red']] as $rating => $meta)
            @php $count = $qualityDist[$rating] ?? 0; $pct = $stats['total_entries'] > 0 ? round($count / $stats['total_entries'] * 100) : 0; @endphp
            <div class="mb-3">
                ...
                <div class="w-full bg-gray-100 rounded-full h-2">
                    <div class="bg-{{ $meta['color'] }}-500 h-2 rounded-full" style="width: {{ $pct }}%"></div>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <form action="{{ route('admin.guru-monitor.adjust-earnings', $guru) }}" method="POST">
                @csrf
                <div class="mb-3">
            ...
            @else
                <div class="space-y-1 max-h-48 overflow-y-auto">
                    ...
                    <div class="flex justify-between text-xs text-gray-600">
                        ...
                        <span class="font-medium">{{ $count }} entri</span>
                    </div>
                    @endforeach
                </div>
            @endif
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-700">{{ $entry->category ?? '-' }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $entry->platform ?? '-' }}</td>
                        <td class="px-4 py-3 text-center">
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-{{ $rc }}-100 text-{{ $rc }}-700">
                                {{ ucfirst($entry->quality_rating ?? '-') }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-600 max-w-xs truncate">{{ Str::limit($entry->input_text, 80) }}</td>
                        <td class="px-4 py-3 text-center text-gray-500 text-xs">{{ $entry->created_at->format('d M Y') }}</td>
                    </tr>
                    @empty
                    <tr>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    @endphp
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-gray-600">{{ $wd->created_at->format('d M Y') }}</td>
                        <td class="px-4 py-3 text-right font-medium text-gray-800">Rp {{ number_format($wd->amount, 0, ',', '.') }}</td>
                        <td class="px-4 py-3 text-center">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-{{ $wc }}-100 text-{{ $wc }}-700">
                                {{ ucfirst($wd->status) }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-gray-500 text-xs">{{ $wd->notes ?? '-' }}</td>
                    </tr>
                    @empty
                    <tr>
