        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">Rp {{ number_format($stats['totalRevenue'], 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">Rp {{ number_format($stats['operatorEarnings'], 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">Rp {{ number_format($stats['platformCommission'], 0, ',', '.') }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-900 mt-1">Rp {{ number_format($stats['pendingWithdrawals'], 0, ',', '.') }}</p>
                </div>
            ...
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-xl font-bold text-gray-400">#{{ $index + 1 }}</td>
                        <td class="px-4 py-3">
                            <div class="text-sm font-medium text-gray-900">{{ $operator->name }}</div>
                            <div class="text-xs text-gray-500">{{ $operator->email }}</div>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-900">{{ $operator->completed_orders }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center">
                                <svg class="w-4 h-4 text-yellow-500 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                                </svg>
                                <span class="text-sm font-medium">{{ number_format($operator->operatorProfile->average_rating ?? 0, 1) }}</span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-sm font-medium text-green-600">
                            Rp {{ number_format($operator->operatorProfile->total_earnings ?? 0, 0, ',', '.') }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
