...
@section('content')
<div class="p-6" x-data="{ correctedOutput: '{{ addslashes($order->result) }}' }">
    <!-- Header -->
    <div class="mb-6">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4 sticky top-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold">{{ $order->category }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="text-sm font-semibold">{{ $order->operator ? $order->operator->name : 'N/A' }}</p>
                    </div>

                    <div>
                        <p class="text-xs text-gray-600">Completed</p>
                        <p class="text-sm font-semibold">{{ $order->completed_at ? $order->completed_at->format('d M Y H:i') : $order->updated_at->format('d M Y H:i') }}</p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex items-center">
                            @for($i = 1; $i <= 5; $i++)
                            <svg class="w-4 h-4 {{ $i <= $order->rating ? 'text-yellow-500' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="lg:col-span-2">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4 mb-4">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <p class="text-sm text-gray-800 whitespace-pre-wrap">{{ $order->result }}</p>
                </div>
            </div>

            <!-- Review Form -->
            <form method="POST" action="{{ route('guru.training.store') }}" class="bg-white rounded-lg border border-gray-200 p-4">
                @csrf
