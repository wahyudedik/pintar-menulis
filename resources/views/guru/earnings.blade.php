...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Rp {{ number_format($stats['earnings_per_entry'], 0, ',', '.') }} per isian training data</p>
        </div>
        @if($stats['available_balance'] >= 10000)
            <a href="{{ route('guru.withdrawal.create') }}"
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <p class="text-2xl font-semibold text-gray-900">{{ number_format($stats['total_entries']) }}</p>
        </div>
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Total Penghasilan</p>
            <p class="text-2xl font-semibold text-blue-600">Rp {{ number_format($stats['total_earnings'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Pending Withdrawal</p>
            <p class="text-2xl font-semibold text-yellow-600">Rp {{ number_format($stats['pending_withdrawal'], 0, ',', '.') }}</p>
        </div>
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <p class="text-xs text-gray-500 mb-1">Saldo Tersedia</p>
            <p class="text-2xl font-semibold text-green-600">Rp {{ number_format($stats['available_balance'], 0, ',', '.') }}</p>
        </div>
    </div>
    ...
    <div class="bg-gradient-to-r from-purple-600 to-indigo-600 rounded-xl p-5 mb-8 flex items-center justify-between">
        <div>
            ...
            <p class="text-purple-100 text-sm">Saldo tersedia: Rp {{ number_format($stats['available_balance'], 0, ',', '.') }}</p>
        </div>
        <div class="flex space-x-2">
            <a href="{{ route('guru.withdrawal.history') }}"
            ...
            @forelse($myEntries as $entry)
                <div class="px-5 py-4 flex items-center justify-between">
                    <div class="flex-1 min-w-0">
                        <p class="text-sm text-gray-900 truncate">{{ Str::limit($entry->input_prompt, 80) }}</p>
                        <div class="flex items-center gap-3 mt-1">
                            <span class="text-xs text-gray-400">{{ $entry->created_at->format('d M Y, H:i') }}</span>
                            <span class="text-xs px-2 py-0.5 rounded-full font-medium
                                {{ $entry->quality_rating === 'excellent' ? 'bg-green-100 text-green-700' :
                    ...
                    <div class="ml-4 text-right">
                        <span class="text-sm font-semibold text-green-600">
                            +Rp {{ number_format($stats['earnings_per_entry'], 0, ',', '.') }}
                        </span>
                    </div>
                </div>
            @empty
