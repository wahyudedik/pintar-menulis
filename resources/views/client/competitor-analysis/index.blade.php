...
<div class="p-6">
    ...
    <div class="mb-6 bg-red-50 border border-red-200 rounded-xl p-4">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <p class="text-sm font-medium text-red-900">{{ $unreadAlertsCount }} Alert Baru!</p>
                    <p class="text-xs text-red-700">Kompetitor ada aktivitas baru</p>
                </div>
            </div>
            <a href="{{ route('competitor-analysis.alerts') }}" 
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 hover:shadow-lg transition competitor-card" 
             ...
             data-competitor-name="{{ $competitor->username }}">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-2">
                            <p class="text-xs text-gray-500 capitalize">{{ $competitor->platform }}</p>
                            @if($competitor->category)
                            <span class="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded">{{ $competitor->category }}</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <!-- Stats -->
            @if($competitor->latestSummary)
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="bg-blue-50 rounded-lg p-3">
                    <p class="text-xs text-blue-600 font-medium">Total Posts</p>
                    <p class="text-lg font-bold text-blue-900">{{ $competitor->latestSummary->total_posts }}</p>
                </div>
                <div class="bg-purple-50 rounded-lg p-3">
                    <p class="text-xs text-purple-600 font-medium">Avg Engagement</p>
                    <p class="text-lg font-bold text-purple-900">{{ number_format($competitor->latestSummary->avg_engagement_rate, 1) }}%</p>
                </div>
            </div>
            ...
            <div class="mb-4 p-3 bg-gray-50 rounded-lg">
                ...
                <p class="text-sm font-medium text-gray-900">{{ $competitor->latestSummary->posting_frequency ?? 0 }} posts/week</p>
                <p class="text-xs text-gray-500">Best time: {{ $competitor->latestSummary->best_posting_time ?? 'N/A' }}</p>
            </div>
            @else
    ...
    <div class="mt-8">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('competitor-analysis.alerts') }}" class="text-sm text-purple-600 hover:text-purple-700">
                Lihat Semua →
            </a>
        </div>
        <div class="bg-white rounded-xl border border-gray-200 divide-y divide-gray-200">
            @foreach($recentAlerts->take(5) as $alert)
            <div class="p-4 hover:bg-gray-50 transition {{ $alert->is_read ? 'opacity-60' : '' }}">
                <div class="flex items-start justify-between">
                    <div class="flex-1">
                        <div class="flex items-center space-x-2 mb-1">
                            <span class="px-2 py-0.5 text-xs rounded-full {{ $alert->alert_type === 'new_post' ? 'bg-blue-100 text-blue-700' : ($alert->alert_type === 'high_engagement' ? 'bg-green-100 text-green-700' : 'bg-purple-100 text-purple-700') }}">
                                {{ ucfirst(str_replace('_', ' ', $alert->alert_type)) }}
                            </span>
                            <span class="text-xs text-gray-500">{{ $alert->triggered_at->diffForHumans() }}</span>
                        </div>
                        <p class="text-sm font-medium text-gray-900">{{ $alert->competitor->username }}</p>
