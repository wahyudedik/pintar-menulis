    ...
    <div class="bg-white rounded-lg border border-gray-200">
        ...
        <div class="p-4">
                    ...
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex-1">
                            ...
                            <p class="text-sm text-gray-600 mb-1">
                                Operator: <span class="font-medium">{{ $order->operator->name }}</span>
                            </p>
                            @else
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-lg font-semibold text-green-600">Rp {{ number_format($order->budget, 0, ',', '.') }}</p>
                        </div>
                    </div>

                    <div class="bg-gray-50 rounded p-3 mb-3">
                        <p class="text-sm text-gray-700">{{ Str::limit($order->brief ?? 'No description', 150) }}</p>
                    </div>

                    <div class="flex items-center justify-between">
                        <div class="text-xs text-gray-600">
                            @if($order->deadline)
                            <span class="font-medium">Deadline:</span> {{ $order->deadline->format('d M Y') }}
                            @endif
                            @if($order->status === 'completed' && $order->completed_at)
                            <span class="ml-3 font-medium">Selesai:</span> {{ $order->completed_at->format('d M Y') }}
                            @endif
                        </div>
            ...
            <div class="text-center py-12">
                ...
                <a href="{{ route('browse.operators') }}" class="text-blue-600 hover:text-blue-700 text-sm">
                    Browse Operators →
                </a>
            </div>
            @endif
