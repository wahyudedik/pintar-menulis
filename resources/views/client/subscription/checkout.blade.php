...
@section('content')
<div class="p-6 max-w-2xl mx-auto" x-data="{
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
        ...
        <div class="space-y-2 text-sm">
            ...
            <div class="flex justify-between py-2 border-b border-gray-100">
                <span class="text-gray-600">Kuota AI</span>
                <span class="font-medium">{{ number_format($package->ai_quota_monthly) }} generate/bulan</span>
            </div>
            <div class="flex justify-between pt-2">
                <span class="font-semibold text-gray-900">Total</span>
                <span class="text-xl font-bold text-red-600">Rp {{ number_format($price, 0, ',', '.') }}</span>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('subscription.pay', $package) }}" method="POST" enctype="multipart/form-data">
        ...
        {{-- Gateway Selection --}}
        <div class="bg-white rounded-2xl border border-gray-200 p-5 mb-6">
            <h2 class="font-semibold text-gray-900 mb-4">Pilih Metode Pembayaran</h2>
            <div class="grid gap-3">
                ...
                <label class="flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition"
                    ...
                    <div class="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center text-xl flex-shrink-0">💳</div>
                    <div class="flex-1">
                        <p class="font-semibold text-gray-900 text-sm">Midtrans</p>
                        <p class="text-xs text-gray-500">Credit Card, GoPay, OVO, VA Bank, QRIS — otomatis</p>
                    </div>
                    <span class="px-2 py-0.5 bg-purple-100 text-purple-700 text-xs font-bold rounded-full">Otomatis</span>
                </label>
                @endif
                ...
                <label class="flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition"
                    ...
                    <div class="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center text-xl flex-shrink-0">⚡</div>
                    <div class="flex-1">
                        <p class="font-semibold text-gray-900 text-sm">Xendit</p>
                        <p class="text-xs text-gray-500">VA Bank, E-Wallet, QRIS, Kartu Kredit — otomatis</p>
                    </div>
                    <span class="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-bold rounded-full">Otomatis</span>
                </label>
                @endif
                ...
                <label class="flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition"
                    ...
                    <div class="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center text-xl flex-shrink-0">🏦</div>
                    <div class="flex-1">
                    ...
                    <span class="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs font-medium rounded-full">Manual</span>
                </label>
                @endif
        ...
        <div x-show="gateway === 'midtrans'" x-cloak
             class="bg-purple-50 border border-purple-200 rounded-2xl p-5 mb-6">
        ...
        <div x-show="gateway === 'xendit'" x-cloak
             class="bg-blue-50 border border-blue-200 rounded-2xl p-5 mb-6">
        ...
        <div x-show="gateway === 'manual_transfer'" x-cloak class="mb-6 space-y-4">

            {{-- Bank Selection --}}
            @if($manualBanks->count() > 0)
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-900 mb-3">Pilih Rekening Tujuan</h2>
                <div class="space-y-2">
            ...
            {{-- Upload Proof --}}
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                <h2 class="font-semibold text-gray-900 mb-1">Upload Bukti Pembayaran</h2>
                <p class="text-xs text-gray-500 mb-4">Screenshot atau foto bukti transfer (JPG/PNG, maks 2MB)</p>
                <label class="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:border-red-400 hover:bg-red-50 transition">
