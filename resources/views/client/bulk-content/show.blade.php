...
@section('title', 'Content Calendar - ' . $calendar->title)
...
<div class="p-6">
    <div class="mb-6">
        ...
        <div class="flex items-start justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">{{ $calendar->title }}</h1>
                <div class="flex items-center gap-3 mt-2">
                    ...
                    <span class="text-sm text-gray-600">{{ count($calendar->content_items) }} konten</span>
                </div>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ route('bulk-content.export', [$calendar, 'csv']) }}" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition" x-data="{ expanded: false }">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                <div class="w-12 h-12 bg-blue-50 rounded-lg flex items-center justify-center">
                                    <span class="text-lg font-bold text-blue-600">{{ $item['day_number'] }}</span>
                                </div>
                                <div>
                                    <p class="font-semibold text-gray-900">{{ $item['day_name'] }}</p>
                                    <p class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($item['scheduled_date'])->format('d M Y') }} • {{ $item['scheduled_time'] }}</p>
                                </div>
                            ...
                            <div class="mt-3">
                                <p class="text-sm text-gray-700 whitespace-pre-wrap" 
                                   :class="expanded ? '' : 'line-clamp-3'">{{ $item['caption'] }}</p>
                                
                                @if(strlen($item['caption']) > 200)
                        ...
                        <div class="flex items-center gap-2 ml-4">
                            <button onclick='editContent({{ $item["day_number"] }}, {{ json_encode($item["caption"]) }}, {{ json_encode($item["scheduled_time"]) }})' 
                                class="px-3 py-1.5 text-sm text-blue-600 hover:bg-blue-50 rounded-lg transition">
                                Edit
                            </button>
                            <button onclick='copyCaption({{ json_encode($item["caption"]) }})' 
...
document.getElementById('editForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch(`{{ route('bulk-content.show', $calendar) }}/update/${dayNumber}`, {
            method: 'POST',
            headers: {
