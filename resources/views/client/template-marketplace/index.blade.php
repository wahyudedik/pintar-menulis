...
<div class="p-6" x-data="templateMarketplace()">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-4">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900 mt-1">{{ auth()->user()->templates()->count() }}</p>
                </div>
    ...
    <div x-show="showFilters" x-cloak x-transition class="mb-6">
        <div class="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <form method="GET" action="{{ route('templates.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Search -->
                <div>
                ...
                <div>
                    ...
                    <select name="category" class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="viral_clickbait" {{ request('category') == 'viral_clickbait' ? 'selected' : '' }}>Viral & Clickbait</option>
                        <option value="event_promo" {{ request('category') == 'event_promo' ? 'selected' : '' }}>Event & Promo</option>
                        <option value="branding_tagline" {{ request('category') == 'branding_tagline' ? 'selected' : '' }}>Branding</option>
                        <option value="monetization" {{ request('category') == 'monetization' ? 'selected' : '' }}>Monetization</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-4 flex gap-2">
                    ...
                    <a href="{{ route('templates.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 text-sm font-medium">
                        Reset
                    </a>
                </div>
            </form>
    ...
    <div class="mb-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-sm text-gray-600">{{ $templates->total() }} templates tersedia</span>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-lg transition">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex-1">
                        <h3 class="font-semibold text-gray-900 mb-1 text-sm">{{ $template->title }}</h3>
                    ...
                    <button @click="toggleFavorite({{ $template->id }})" 
                            :class="favorites.includes({{ $template->id }}) ? 'text-red-500' : 'text-gray-300'"
                ...
                <div class="flex items-center justify-between mb-3 text-xs">
                    <div class="flex items-center gap-1">
                        ...
                        <span class="font-medium">{{ number_format($template->rating_average, 1) }}</span>
                        <span class="text-gray-500">({{ $template->total_ratings }})</span>
                    </div>
                    <span class="text-gray-500">{{ $template->usage_count }}x digunakan</span>
                </div>
            ...
            <div class="col-span-3 text-center py-12 bg-white rounded-lg border border-gray-200">
                ...
                <a href="{{ route('templates.create') }}" class="inline-block px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
