...
<div class="p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-gray-500">Project: {{ $project->business_name }}</p>
        </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200">
        ...
        <div class="p-6">
            <form action="{{ route('projects.content.store', $project) }}" method="POST" id="contentForm">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        ...
                        <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('type') border-red-500 @enderror" name="type" id="type" required>
                            <option value="">Pilih Jenis Konten</option>
                            <option value="caption" {{ old('type') == 'caption' ? 'selected' : '' }}>Caption Media Sosial</option>
                            <option value="article" {{ old('type') == 'article' ? 'selected' : '' }}>Artikel/Blog Post</option>
                            <option value="ad_copy" {{ old('type') == 'ad_copy' ? 'selected' : '' }}>Iklan</option>
                            <option value="email" {{ old('type') == 'email' ? 'selected' : '' }}>Email Marketing</option>
                            <option value="product_desc" {{ old('type') == 'product_desc' ? 'selected' : '' }}>Deskripsi Produk</option>
                        </select>
                        @error('type')
                    ...
                    <div>
                        ...
                        <select class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('platform') border-red-500 @enderror" name="platform" id="platform">
                            ...
                            <option value="whatsapp" {{ old('platform') == 'whatsapp' ? 'selected' : '' }}>WhatsApp Status</option>
                            ...
                            <option value="website" {{ old('platform') == 'website' ? 'selected' : '' }}>Website/Blog</option>
                        </select>
                        @error('platform')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('title') border-red-500 @enderror" 
                ...
                <div class="mb-6">
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('content') border-red-500 @enderror" 
                              name="content" id="content" rows="8" 
                              placeholder="Tulis konten Anda di sini..." required>{{ old('content') }}</textarea>
                    @error('content')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="text" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('tags') border-red-500 @enderror" 
                ...
                <div class="mb-6">
                    ...
                    <textarea class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('notes') border-red-500 @enderror" 
                              ...
                              placeholder="Tambahkan catatan atau instruksi untuk reviewer...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-between items-center">
                    ...
                    <a href="{{ route('projects.show', $project) }}" class="text-gray-600 hover:text-gray-800 text-sm">
                        Batal
                    </a>
                </div>
            </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('generateAI').addEventListener('click', function() {
        ...
        fetch('{{ route("api.ai.generate-content") }}', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                business_context: {
                    name: '{{ addslashes($project->business_name) }}',
                    type: '{{ addslashes($project->business_type) }}',
                    description: '{{ addslashes($project->business_description) }}',
                    audience: '{{ addslashes($project->target_audience ?? '') }}',
                    brand_tone: '{{ addslashes($project->brand_tone ?? '') }}'
                }
            })
        })
