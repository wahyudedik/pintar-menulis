...
<?php $__env->startSection('title', 'Team Management - ' . $project->business_name); ?>
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <nav class="flex items-center space-x-2 text-sm text-gray-500 mb-2">
                <a href="<?php echo e(route('projects.index')); ?>" class="hover:text-gray-700">Projects</a>
                <span>/</span>
        ...
        <?php if($project->canUserApprove(auth()->user())): ?>
        ...
        <div class="bg-white rounded-lg border border-gray-200">
            ...
            <div class="p-6">
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500"><?php echo e($project->user->email); ?></p>
                            </div>
                        </div>
                    ...
                    <div class="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">Joined <?php echo e($member->joined_at->diffForHumans()); ?></p>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            <?php if($project->canUserApprove(auth()->user())): ?>
                            <select @change="updateMemberRole(<?php echo e($member->id); ?>, $event.target.value)" 
                            ...
                            <button @click="removeMember(<?php echo e($member->id); ?>)" 
                            ...
                            <?php else: ?>
                            <span class="px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded-full"><?php echo e(ucfirst($member->role)); ?></span>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <?php if($members->where('status', 'accepted')->isEmpty()): ?>
                    ...
                    <div class="flex items-center justify-between p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <div class="flex items-center space-x-3">
                            <div class="w-10 h-10 bg-yellow-100 rounded-full flex items-center justify-center">
                                <span class="text-sm font-medium text-yellow-600"><?php echo e(substr($invitation->user->name, 0, 1)); ?></span>
                            </div>
                            <div>
                                <p class="text-sm font-medium text-gray-900"><?php echo e($invitation->user->name); ?></p>
                                <p class="text-xs text-gray-500"><?php echo e($invitation->user->email); ?></p>
                                <p class="text-xs text-gray-400">Invited <?php echo e($invitation->invited_at->diffForHumans()); ?> by <?php echo e($invitation->invitedBy->name); ?></p>
                            </div>
                        </div>
                        <div class="flex items-center space-x-2">
                            <span class="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full"><?php echo e(ucfirst($invitation->role)); ?></span>
                            <span class="px-2 py-1 bg-orange-100 text-orange-800 text-xs rounded-full">Pending</span>
                        </div>
                    </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>

                    <?php if($pendingInvitations->isEmpty()): ?>
                    <div class="text-center py-8">
                        <svg class="mx-auto h-12 w-12 text-gray-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function teamManagement() {
    return {
        ...
        async inviteMember() {
            ...
            try {
                const response = await fetch(`/projects/<?php echo e($project->id); ?>/team/invite`, {
                    method: 'POST',
                    headers: {
        ...
        async updateMemberRole(memberId, newRole) {
            try {
                const response = await fetch(`/projects/<?php echo e($project->id); ?>/team/${memberId}/role`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
        ...
        async removeMember(memberId) {
            ...
            try {
                const response = await fetch(`/projects/<?php echo e($project->id); ?>/team/${memberId}`, {
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                    }
...
<?php echo $__env->make('layouts.client', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\PROJEKU\pintar-menulis\resources\views/projects/collaboration/index.blade.php ENDPATH**/ ?>